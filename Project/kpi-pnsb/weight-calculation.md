...
## TWO editors of besar, ONE shared rescale (2026-06-29)

`bsc_perspectives.weight` is now editable from **two** places, and BOTH must enforce the 100% total AND auto-rescale:
1. **KPI Library** — `Kpis/Index` BSC Weights modal (original).
2. **Calculation Settings** — `Settings\Calculation` BSC Dimensions panel (added this session; also edits description + sort_order).

The rescale logic was **extracted** to a shared static `BscPerspective::rescaleKpiWeights(int $perspectiveId, float $oldBesar, float $newBesar)` — both editors call it. **Bug fixed (2026-06-29):** the Calculation Settings panel originally did a BLUNT `update(['weight'=>...])` with no rescale + only a soft "warn if ≠100%". That desynced the KPI Library (KPIs still summed to the old besar) — visible as a mismatched "Suggested weight total" on small weights. Now it hard-blocks ≠100% and rescales, identical to the KPI Library. **Watch-out:** never reintroduce a raw besar update anywhere — always go through `BscPerspective::rescaleKpiWeights()` + the 100% gate.

## AUTO-RESIZE on big-weight change (the calculation Amirul asked to note)

When super-admin edits a perspective's besar from `oldBesar` → `newBesar` (Kpis/Index BSC Weights editor OR Calculation Settings BSC Dimensions panel), EVERY KPI in that perspective is rescaled by the SAME ratio:
